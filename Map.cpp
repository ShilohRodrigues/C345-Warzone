...
Player *ta = new Player(); //default player
...
Territory::Territory(int idTerritory, const string& territoryName, int territoryContinent){
    ...
    playerInPossession = std::make_unique<Player>(*ta);
}
//Parameterized
Territory::Territory(int idTerritory, const string& territoryName, int territoryContinent, const Player& player,int armies) {
    id = std::make_unique<int>(idTerritory);
    name = std::make_unique<string>(territoryName);
    continentId = std::make_unique<int>(territoryContinent);
    playerInPossession = std::make_unique<Player>(player);
...
Territory::Territory(const Territory &territory) {
    ...
    playerInPossession = std::make_unique<Player>(*territory.playerInPossession);
...
ostream& operator<<(ostream &os, const Territory &t) {
    ...
    if(t.playerInPossession == nullptr) {
        ...
        os << "Territory Name: " << *t.name <<
           ...
           << ", Player on territory: " << t.playerInPossession->getName()
           << ", Armies: " << *t.armyCnt << endl;
    }
    return os;
}
